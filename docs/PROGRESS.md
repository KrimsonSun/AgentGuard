...
## Day 2 · 2026-07-14（真机端到端成功 + 多轮修复）
- **真机首次完整成功**（单号40）：gpt-4o-mini + 2工具(update_slots/finish_registration)，回访自动命中——
  凭手机号认出张先生 →"还是来蓝色鲸鱼送货吧"→"对的"→放行，**全程不重复问**（题目理想 ~10s 回访）；数据真落库。
- 修复链（决策16/17）：flash-lite 工具不稳→换 gpt-4o-mini（数据 3/3 胜 gemini-3.1-flash-lite/2.5-flash）；
  幻觉收尾→原子 finish_registration；称呼刷屏→偶尔用；回访顺序 bug→并入 update_slots 后端自动查；STT→Azure中文。
- **剩余真问题=延迟**：Vapi 填充语("就一秒")源于 LLM 跳 `Vapi→隧道→Mac→OpenRouter`；根治=部署近处 / C(代理内循环)。
- 小瑕疵：Azure 偶漏车牌省份字("鄂AVK696"→"2avk696")；回访命中可改用历史车牌。
- 待办：并发压测（用户下一步）。
