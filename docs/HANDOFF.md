...
**验证**(真实 gpt-4o-mini + Neon):问"张师傅来了多少次"→ 列出张先生(6)/张师傅(4)两候选反问是哪位;报手机指明→ `find_visitors→visitor_report`,答"张先生6次:送货5次、给隔壁送货1次+车牌"。✅

**意义**：又一个"结构级可靠"的应用——**读操作的正确性靠"先定位唯一身份再作答"的协议保证,不靠模型自觉数对**。呼应决策17(能用代码保证的别指望模型)。
